// live-shifts.js — working weekly rota + generator + McAssist shift commands for the redesigned UI
(function () {
  ...
  function parseStation(text) {
    const t = norm(text);
    const map = {
      fries: "Fries", kitchen: "Kitchen", front: "Front Counter", counter: "Front Counter", drive: "Drive-thru",
      lobby: "Lobby", line: "Line", grill: "Grill", stock: "Stock", drinks: "Drinks", runner: "Runner", chicken: "Chicken"
    };
  ...
  function generateWeek(offset = currentWeekOffset, silent = false, targetList = null) {
    const existing = targetList || loadShifts().filter((s) => !getWeekDays(offset).map(toISO).includes(s.date));
    const days = getWeekDays(offset);
    let idx = offset * 2;
    ...
    days.forEach((d) => {
      ...
      const blocks = demand === "high"
        ? [{ start: "09:00", end: "17:00" }, { start: "12:00", end: "20:00" }, { start: "16:00", end: "23:00" }]
        : demand === "low"
          ? [{ start: "10:00", end: "18:00" }, { start: "15:00", end: "22:00" }]
      ...
      blocks.forEach((block, b) => {
        ...
        for (let slot = 0; slot < needed; slot++) {
          ...
          existing.push({
            id: `shift_${date}_${block.start}_${person.id}_${slot}_${Math.random().toString(16).slice(2, 7)}`,
            date,
            start: block.start,
            end: block.end,
            userId: person.id,
            ...
            role: person.role,
            station,
            risk: slot === 0 ? "Covered" : demand === "high" ? "Peak" : "Normal",
            generatedByAI: true
          });
        }
        idx += 1;
  ...
  function renderWeekTabs() {
    ...
    tabs.querySelectorAll("button").forEach((btn) => btn.addEventListener("click", () => {
      ...
      localStorage.setItem(WEEK_KEY, String(currentWeekOffset));
      renderAllSchedules();
    }));
  }
  ...
  function renderScheduleCards() {
    ...
    box.innerHTML = `
      <div class="between wrap"><div><h3>${currentWeekOffset === 0 ? "This week" : currentWeekOffset === 1 ? "Next week" : "Week after"}</h3><p class="muted">${weekLabel(currentWeekOffset)} • ${week.length} shifts scheduled</p></div><span class="pill ok">${isCrew ? "Your view" : "Manager view"}</span></div>
      <div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(210px,1fr));margin-top:14px;">
        ${days.map((d) => {
          const iso = toISO(d);
          let list = week.filter((s) => s.date === iso);
          if (isCrew) list = list.filter((s) => s.userName.toLowerCase().startsWith((user.name || "").toLowerCase()[0]) || s.userId === user.id);
          return `<article class="mini"><h3>${dayNames[d.getDay()]} ${d.getDate()}</h3>${list.length ? list.map((s) => `<div class="between" style="border-top:2px dashed rgba(35,18,10,.15);padding-top:8px;margin-top:8px;"><span><b>${esc(s.userName)}</b><br><small class="muted">${s.start}-${s.end} • ${esc(s.station)}</small></span><span class="pill ${s.risk === "Peak" ? "warn" : "ok"}">${s.risk}</span></div>`).join("") : `<p class="muted" style="margin-top:8px;">No shifts.</p>`}</article>`;
        }).join("")}
      </div>`;
  }
  ...
  function renderManagerTools() {
    ...
    box.innerHTML = `
      <div class="between wrap"><div><h3>Working shift tools</h3><p class="muted">Generate a week, create a manual shift, then view it on the rota.</p></div><span class="pill ok">Live in browser</span></div>
      <div class="grid two" style="margin-top:14px;">
        <div class="mini">
          <h3>Generate week</h3>
          <div class="field"><label>Week</label><select id="liveGenWeek"><option value="0">This week</option><option value="1">Next week</option><option value="2">Week after</option></select></div>
          <button id="liveGenerateBtn" class="btn" type="button">🤖 Generate fair week</button>
          <button id="liveClearWeekBtn" class="btn alt" type="button" style="margin-top:8px;">Clear this week</button>
        </div>
        <div class="mini">
          <h3>Create manual shift</h3>
          <div class="field"><label>Crew</label><select id="liveCrew">${crew.map((c) => `<option value="${c.name}">${c.name} - ${c.role}</option>`).join("")}</select></div>
          <div class="grid two"><div class="field"><label>Date</label><input id="liveDate" type="date" value="${today}"></div><div class="field"><label>Station</label><select id="liveStation">${stationPlan.map((s) => `<option>${s}</option>`).join("")}</select></div></div>
          <div class="grid two"><div class="field"><label>Start</label><input id="liveStart" type="time" value="14:00"></div><div class="field"><label>End</label><input id="liveEnd" type="time" value="22:00"></div></div>
          <button id="liveCreateBtn" class="btn" type="button">➕ Create shift</button>
        </div>
      </div>`;

    $("liveGenerateBtn")?.addEventListener("click", () => {
      const offset = Number($("liveGenWeek")?.value || 0);
      currentWeekOffset = offset;
      localStorage.setItem(WEEK_KEY, String(offset));
      generateWeek(offset, false);
      renderAllSchedules();
    });
    $("liveClearWeekBtn")?.addEventListener("click", () => {
      const offset = Number($("liveGenWeek")?.value || currentWeekOffset);
      const dates = new Set(getWeekDays(offset).map(toISO));
      saveShifts(loadShifts().filter((s) => !dates.has(s.date)));
      currentWeekOffset = offset;
      localStorage.setItem(WEEK_KEY, String(offset));
      renderAllSchedules();
      toast("Week cleared");
    });
    $("liveCreateBtn")?.addEventListener("click", () => {
      const reply = addManualShift({ name: $("liveCrew").value, date: $("liveDate").value, start: $("liveStart").value, end: $("liveEnd").value, station: $("liveStation").value });
      toast(reply);
    });
  }
  ...
  function handleAi(text) {
    ...
    if (q.includes("generate") && (q.includes("shift") || q.includes("rota") || q.includes("schedule"))) {
      const offset = q.includes("next week") ? 1 : q.includes("week after") ? 2 : currentWeekOffset;
      currentWeekOffset = offset;
      localStorage.setItem(WEEK_KEY, String(offset));
      generateWeek(offset, true);
      renderAllSchedules();
      return `Done ✅ I generated a fair rota for ${weekLabel(offset)}. You can see it on the Shifts page and Shift Creator page.`;
    }
    if ((q.includes("show") || q.includes("see") || q.includes("what")) && (q.includes("shift") || q.includes("rota") || q.includes("schedule"))) {
      const offset = q.includes("next week") ? 1 : q.includes("week after") ? 2 : currentWeekOffset;
      return weekSummary(offset);
    }
    if ((q.includes("create") || q.includes("add") || q.includes("make")) && q.includes("shift")) {
      const name = parseName(text);
      const time = parseTimeRange(text);
      if (!name || !time) return "Try: create shift for Amelia tomorrow 9-5 fries.";
      return addManualShift({ name, date: parseDate(text), start: time.start, end: time.end, station: parseStation(text) });
    }
    if ((q.includes("delete") || q.includes("remove") || q.includes("cancel")) && q.includes("shift")) return deleteShiftFromText(text);
    if (q.includes("rush") || q.includes("risk")) return "Dinner rush risk: check Front Counter and Fries between 17:30-19:30. If you are short, move one trained crew member from Lobby to Front Counter and keep Leo as shift lead.";
    if (q.includes("training")) return "Training priority: food safety basics, fries refresh, then front counter service practice. Use the Training page to show progress and manager sign-off.";
    ...
    return "I can generate shifts, show this week or next week rota, create/delete shifts, explain training gaps, and help with your corporate pitch.";
  }
  ...
  function patchChat() {
    ...
    if (chips) {
      chips.innerHTML = ["Show this week shifts", "Generate next week shifts", "Create shift for Amelia tomorrow 9-5 fries", "Pitch this to corporate"].map((c) => `<button type="button">${c}</button>`).join("");
    ...
    form.addEventListener("submit", (e) => {
      ...
      addMsg("user", text);
      input.value = "";
      setTimeout(() => addMsg("bot", handleAi(text)), 160);
    }, true);
  }
  ...
  function init(page) {
    ...
    if (page === "shifts") {
      const picker = $("crewPicker");
      if (picker) picker.innerHTML = crew.map((c) => `<option>${c.name} - ${c.role}</option>`).join("");
    }
    patchChat();
  }

  window.McLive = { init, generateWeek, renderAllSchedules, weekSummary };
})();
