...
// UPDATED:
// - Uses stores/{storeId}/shifts (lowercase) like main.js
// - Adds UK break rules so hours match dashboard
...
const SHIFTS_COLLECTION = "shifts"; // MUST match main.js
...
// --- same break rules as main.js ---
// Under 5h → 15m, 5–8h → 30m, 8h+ → 45m
...
async function loadShiftsFromFirestore(storeId) {
  allShifts = [];
  try {
    const col = collection(db, "stores", storeId, SHIFTS_COLLECTION);
  ...
  days.forEach((d) => {
    ...
    if (!dayShifts.length) {
      ...
      if (!myShifts.length) {
        ...
        dayContent = myShifts
          .map((s) => {
            const raw = parseShiftHours(s.start, s.end);
            const br = calculateBreakMinutes(raw);
            const paid = (raw - br / 60).toFixed(2);
            return `
      ...
      const myMain = myShifts
        .map((s) => {
          ...
          const paid = (raw - br / 60).toFixed(2);
          return `
          <li style="display:flex;flex-direction:column;gap:2px;" data-shift-id="${s.id}">
            <div style="display:flex;align-items:center;justify-content:space-between;gap:6px;">
              <div>
                <span>${s.start}–${s.end}</span>
...
function attachDeleteHandlers(canManageShifts) {
  ...
  buttons.forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      try {
        await deleteDoc(doc(db, "stores", currentStoreId, SHIFTS_COLLECTION, id));
        await loadShiftsFromFirestore(currentStoreId);
...
function renderShiftManageTools(canManageShifts, storeId) {
  ...
  if (createShiftBtn) {
    createShiftBtn.addEventListener("click", async () => {
      ...
      try {
        await addDoc(collection(db, "stores", storeId, SHIFTS_COLLECTION), {
          date,
          start,
          end,
          userId,
