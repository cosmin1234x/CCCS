...
function parseDates(text) {
  ...
  if (m) {
    ...
    const end = new Date(`${m[2] || m[1]}T12:00:00`);
    const out = [];
...
async function loadShifts(dates) {
  ...
  snap.forEach((docSnap) => {
    ...
    if (wanted.has(d.date)) shifts.push({ id: docSnap.id, ...d });
  });
...
function addStyles() {
  ...
  s.textContent = `
    .rota-review-card{margin:14px 0;padding:16px;border:2px solid #ffc300;border-radius:22px;background:linear-gradient(135deg,#fffdf2,#fff4c7);box-shadow:0 14px 36px rgba(120,53,15,.14)}
    .rota-review-head{display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap;margin-bottom:10px}.rota-review-title{font-size:1rem;font-weight:900;color:#8b0000;margin:0}.rota-review-sub{font-size:.78rem;color:#7c2d12;margin-top:4px}
    .rota-review-actions{display:flex;gap:8px;flex-wrap:wrap}.rota-review-btn{border:0;border-radius:999px;padding:9px 12px;font-weight:850;cursor:pointer;background:#dc0019;color:white}.rota-review-btn.light{background:white;color:#991b1b;border:1px solid #f59e0b}.rota-review-btn.dark{background:#111827;color:white}
    .rota-review-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px;margin-top:12px}.rota-box{background:white;border:1px solid #facc15;border-radius:16px;padding:10px;min-height:80px}.rota-box h4{margin:0 0 6px;font-size:.82rem;color:#991b1b;font-weight:900}.rota-line{font-size:.74rem;border-top:1px dashed #fde68a;padding-top:5px;margin-top:5px;color:#111827}.rota-pill{display:inline-flex;margin:2px 4px 2px 0;border-radius:999px;padding:4px 7px;background:#fff7ed;border:1px solid #fed7aa;font-size:.72rem;font-weight:800;color:#9a3412}.rota-problem{font-size:.74rem;color:#991b1b;margin:4px 0}
  `;
...
function stats(shifts, dates) {
  ...
  shifts.forEach((s) => {
    ...
    byDay[s.date].push(s);
    if (stationCount[s.station] !== undefined) stationCount[s.station]++;
    if (!people[s.userId]) people[s.userId] = { name: s.userName || "Crew", days: new Set(), shifts: 0 };
    people[s.userId].days.add(s.date);
    people[s.userId].shifts++;
  });
  Object.entries(byDay).forEach(([date, list]) => {
    const usedPeople = new Set();
    list.forEach((s) => {
      if (usedPeople.has(s.userId)) problems.push(`${s.userName || "Crew"} has more than 1 shift on ${date}`);
      usedPeople.add(s.userId);
    });
  });
...
async function renderReview(sourceText) {
  ...
  if (!card) {
    ...
    card.id = "rotaReviewCard";
    card.className = "rota-review-card";
    host.parentNode.insertBefore(card, host.nextSibling);
  }
  const dayHtml = dates.map((d) => {
    const key = iso(d);
    const list = st.byDay[key] || [];
    return `<div class="rota-box"><h4>${d.toLocaleDateString(undefined,{weekday:"short",day:"numeric",month:"short"})}</h4>${list.length ? list.map((x) => `<div class="rota-line"><strong>${esc(x.userName || "Crew")}</strong><br>${esc(x.start)}–${esc(x.end)} · ${esc(x.station || "Station")}</div>`).join("") : `<div class="rota-line">No shifts</div>`}</div>`;
  }).join("");
...
function start() {
  ...
  new MutationObserver(() => {
    const last = [...chat.querySelectorAll(".msg-bot .bubble")].at(-1);
    const text = last?.innerText || "";
    if (text.includes("Smart shifts generated")) setTimeout(() => renderReview(text), 600);
